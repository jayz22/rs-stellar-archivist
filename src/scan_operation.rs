...
impl Operation for ScanOperation {
    ...
    async fn process_history(&self, path: &str) -> Result<HistoryOutcome, StorageError> {
        let buffer = {
            ...
            crate::storage::download_buffer(&self.src_store, path).await?
        };
        crate::metrics::add_bytes(crate::metrics::Phase::HistoryFetch, buffer.len() as u64);
        crate::metrics::record_file(buffer.len() as u64);
        let state = {
