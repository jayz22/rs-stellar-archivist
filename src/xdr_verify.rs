...
/// Streaming core for every XDR file: read gzipped bytes from `reader`, optionally
/// tee the still-compressed bytes to `writer`, gzip-decode to a buffer, and run
/// `parse` on it ALL inside one spawned task (approach E — gzip decode AND the XDR
/// parse/hash leave the orchestration task; the caller only feeds chunks). Returns
/// the parse result alongside the **unclosed** sink (`None` when `writer` is
/// `None`); the caller commits a write only after `parse` succeeds
/// (parse-before-commit), so a parse failure never leaves committed corrupt data.
///
    ...
    T: Send + 'static,
{
    ...
    crate::metrics::add_bytes(crate::metrics::Phase::XdrDecompress, len);
    crate::metrics::record_file(len);
...
pub async fn verify_and_write_xdr(
    ...
    // Decode + parse run together in the spawned task (approach E). The closure
    // captures only owned data (the logical `path` for file-type detection and
    // `cp`) so it is `Send + 'static`. Classify on `path`, NOT `write_path`, so a
    // `.tmp` sibling is never misread as an unrecognized type.
    let cp = history_format::checkpoint_from_path(path);
    let path_owned = path.to_string();
