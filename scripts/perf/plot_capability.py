#!/usr/bin/env python3
"""Plot the verify capability graph from perf-results/capability.csv.
Two panels: (1) throughput (MB/s) vs cores available, with an ideal-linear
reference for the winner; (2) cores actually used vs cores available.
Usage: plot_capability.py [perf-results/capability.csv]"""
import sys, os, csv, collections
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt

path = sys.argv[1] if len(sys.argv) > 1 else "perf-results/capability.csv"
...
for label in sorted(data):
    ...
    used = [float(data[label][c]["cores_mean"]) for c in xs]
...
ax2.set_ylabel("mean cores actually used")
ax2.set_title("Core utilization")
ax2.grid(True, alpha=0.3); ax2.legend()

out = "perf-results/plots/verify_capability.png"
